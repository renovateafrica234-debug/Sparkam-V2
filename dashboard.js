  ...
  const sparkTheBrain = async () => {
    ...
    try {
      const response = await fetch('/api/promote', {
        ...
        body: JSON.stringify({
          fullName: user?.fullName || "Zeeter Oliver",
          email: user?.email || "zeeteroliver@gmail.com",
          artistName: user?.artistName || "Zeeter Oliver",
          budget: promoBudget,
  ...
  return (
    <div style={styles.dashboardContainer}>
      <aside style={styles.sidebar}>
        <h2 style={styles.logo}>SPARKAM.</h2>
        <nav style={styles.nav}>
          <div style={styles.navItemActive}>AI ANALYSIS</div>
          <div style={styles.navItem}>CONTENT CREATOR</div>
